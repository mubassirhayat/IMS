<?php
	...
	if(isset($_POST['insert'])){
		...
		$GWExpiry = $_POST['GWExpiry'];
		$query = "INSERT INTO stock (
					item_name, item_quantity, date_item_added, category_id, item_manufacturer, item_g_w_check, item_upc, item_expiry_date, item_purchase_price, item_sale_price, item_g_w_expiry_period
				) VALUES (
					'{$itemName}', '{$quantity}', '{$dateAdded}', '{$categoryId}', '{$manufacturer}', '{$gwc}', '{$upc}', '{$expiryDate}', '{$purchasePrice}', '{$salePrice}', '{$GWExpiry}'
				)";
...
<script type="application/javascript">
	...
	function urllll() {
		popUP('views/content/insert/category.inc.php', 'New Category',400,160);
	}
</script>
<?php
if (!empty($errors)) {
...
<table>
  ...
  <tr>
    <td colspan="2">
    	<div align="center">
			<label><input class="checkboxStyle" name="gwc" type="radio" value="g"><span>Guarantee</span></label>
    		<label><input class="checkboxStyle" name="gwc" type="radio" value="w"><span>Warranty</span></label>
		</div>
    </td>
  </tr>
  <tr>
  	<td>Guarantee/Warranty Expiry Period: </td>
	<td><input class="textBoxStyle" name="GWExpiry" id="GWExpiry" type="number" /></td>
  </tr>
  <tr>
